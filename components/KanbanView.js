...
export default function KanbanView() {
  ...
  return (
    ...
    <div className="grid grid-cols-4 gap-4 items-start">
      {columns.map((col) => (
        <div key={col.label} className="bg-stone-100 rounded-2xl p-3">
          <div className="flex items-center justify-between px-2 mb-3">
            <h2 className="text-xs font-bold uppercase tracking-wider text-stone-500 capitalize">
              {col.label}
            </h2>
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <input
                        ...
                        className="h-3.5 w-3.5 rounded border-stone-300 accent-stone-700 shrink-0 mt-0.5 cursor-pointer"
                      />
                      <Link href={`/tasks/${task.id}/edit`} className="text-sm font-medium text-stone-800 leading-snug flex-1 hover:underline">
                        {task.title}
                      </Link>
                      <div
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span
                        className={`text-[10px] font-mono ${
                          isOverdue ? "text-red-400 font-medium" : "text-stone-400"
                        }`}
                      >
